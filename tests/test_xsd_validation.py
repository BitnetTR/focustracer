...
uygunluğunu doğrular (v1, v2, v2.1, v2.2).
"""
...
class XSDValidationTests(unittest.TestCase):
    ...
    def test_simple_function_trace_validates_v22(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "simple_v22.xml"
    ...
    def test_loop_compaction_validates_v22(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "loop_v22.xml"
    ...
    def test_exception_with_traceback_validates_v22(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "exc_v22.xml"

            def target():
                x = 1
                raise ValueError("v22 test error")
    ...
    def test_scope_timing_attributes_validates_v22(self):
        """v2.2: scope start_time/end_time/duration attributes are valid."""
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "timing_v22.xml"
    ...
    def test_thread_trace_validates_v22(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "thread_v22.xml"
            ...
            t = threading.Thread(target=worker, name="Worker-V22")
            with TraceContext(
                output_file=str(out),
                output_format="xml",
                schema_version="2.2",
                enable_threading=True,
                target_functions=["worker"],
                target_thread_names=["Worker-V22"],
            ):
                t.start()
                t.join()
